...
RUN pip install pyzmq jinja2 tornado && cd /tmp && git clone https://github.com/c3h3/ipython.git && cd /tmp/ipython &&  git checkout not-open-in-new-window && python setup.py install && rm -rf /tmp/ipython
